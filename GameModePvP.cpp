...
class GameModePvP{
    ...
    int checkMate(int Board[][8],Pos PosA[],Pos PosB[]) {
        ...
        for (int i = 0; i < 16; i++) {
            vector<Location> P = ValidMove(i, Board, PosB);
            for (int i = 0; i < P.size(); i++) {
                if (P[i].x == PosA[15].x && P[i].y == PosA[15].y)
                    return 1;
            }
        }
        for (int i = 0; i < 16; i++) {
            vector<Location> P = ValidMove(i, Board, PosA);
            for (int i = 0; i < P.size(); i++) {
                if (P[i].x == PosB[15].x && P[i].y ==PosB[15].y)
                    return -1;
            }
        }
        return 0;
    ...
    int endGameP2() {
        int arr[8][8];
        for (int i = 0; i < 16; i++) {
            vector<Location> P = ValidMove(i, Board, Player1.PosB);
            for (int j = 0; j < P.size(); j++) {
                ...
                if (arr[P[j].x][P[j].y] > 0)
                {
                    for (int k = 0; k < 16; k++)
                    {
                        if ((a[k].x == P[j].x) && (a[k].y == P[j].y))
                        {
                            a[k].value = -1;
                            break;
                        }
    ...
    int endGameP1() {
        int arr[8][8];
        for (int i = 0; i < 16; i++) {
            vector<Location> P = ValidMove(i, Board, Player2.PosA);
            for (int j = 0; j < P.size(); j++) {
                CopyMang(Board, arr);
                Pos a[16];
                CopyPos(Player2.PosA, a);
